...
public class Lemming {
            ...
            case FALLER:
                ...
                if (!crossedLowerBorder()) {
                    ...
                    if (canFloat && counter >= FALL_DISTANCE_FLOAT) {
                        ...
                        if (counter > LemGame.getLevel().getMaxFallDistance()) {
            ...
            case FRIER:
                {
                	...
                	if (LemGame.getLevel().getStyleName().equals("bubble")) {
            ...
            switch (s & (Stencil.MSK_TRAP | Stencil.MSK_EXIT)) {
                case Stencil.MSK_TRAP_LIQUID:
                    if (type != Type.DROWNER) {
                        SpriteObject spr = LemGame.getLevel().getSprObject(object);
                        if (spr != null) {
                ...
                case Stencil.MSK_EXIT:
                    ...
                    if (Core.player.isMaximumExitPhysics() || LemGame.isDirectDrop()) {
                        switch (newType) {
                           case FALLER:
                           case SPLATTER:
                           case CLIMBER:
    ...
    public void playVisualSFX(Sound.Effect e) {
        LemGame.sound.playVisualSFX(e, footX(), midY()); //NOTE: footX was midX
    }
    ...
    private boolean turnedByBlocker() {
        ...
        if (BooleanUtils.toBoolean(s & Stencil.MSK_TURN_LEFT) && dir == Direction.RIGHT) {
            int id = objectFoot();
            if (id >= 0) {
                SpriteObject spr = LemGame.getLevel().getSprObject(id);
                LemGame.sound.playVisualSFX(spr);
            }
            dir = Direction.LEFT;
            return true;
    ...
    private void addExplosion() {
        LemGame.addExplosion(footX(), midY()); //note: footX was midX()
    }
    ...
    private boolean canBash() {
        ...
        for (int i = 18; i < 22; i++) {
            ...
            if (BooleanUtils.toBoolean(LemGame.getStencil().getMask(xb, ypos) & Stencil.MSK_BRICK)) {
                bricks++;
            }
        }
        return bricks > 0;
    ...
    private boolean canBashSteel(final boolean playSound) {
        ...
        int yMax = y - (LemGame.getLevel().getClassicSteel()
    ...
    private boolean canDig(final boolean playSound) {
        ...
        for (int i = 0; i < 2; i++) {
            for (int j = (classicSteel ? 0 : -4); j < (classicSteel ? 1 : 6); j++) {
                int ym = y + i;
                int xm = x + j;
                int sval = LemGame.getStencil().getMask(xm, ym);
                boolean hitSteel = BooleanUtils.toBoolean(sval & Stencil.MSK_STEEL);
                boolean hitOneWayUp = BooleanUtils.toBoolean(sval & Stencil.MSK_ONE_WAY_UP);
                if (BooleanUtils.toBoolean(sval & Stencil.MSK_BRICK) && (hitSteel || hitOneWayUp)) {
                    if (playSound) {
                        SpriteObject spr = LemGame.getLevel().getSprObject(LemGame.getStencil().getMaskObjectID(xm, ym));
    ...
    private boolean canMine(final boolean start, final boolean playSound) {
        ...
        if (!start) {
            ...
            yMin = y - 2; // account for 2px vertical movement at frame 0
            yMax = y + 1;
    ...
    private int freeBelow(final int step) {
        ...
        Stencil stencil = LemGame.getStencil();
        int yb = y;
        pos = x + yb * LemGame.getWidth(); // line below the lemming
        for (int i = 0; i < step; i++) {
            if (yb + i >= LemGame.getHeight()) {
...
//    }
    ...
    public static void loadLemmings() throws ResourceException {
        ...
        lemmings.clear();
        // read lemmings
        LemGame.setLemmNames(p.get("names", "Lemming"));
        LemGame.setLemmNamesPlural(p.get("names_plural", "Lemmings"));
            ...
            if (type.frames > 0) {
                ...
                if (bidirectional) {
                    resource = Core.findResource(
                            ...
                            Core.IMAGE_EXTENSIONS);
                    LemImage sourceImgLeft = Core.loadLemImage(resource);
...
class LemmingResource {
    ...
    private final List<List<LemImage>> img = new ArrayList<>(2);

    /** for recoloring purposes */
    private final List<List<LemImage>> originalColorImg = new ArrayList<>(2);
    ...
    LemmingResource() {
        ...
        List<LemImage> imgTemp = new ArrayList<>(1);
        imgTemp.add(ToolBox.createLemImage(width, height, Transparency.BITMASK));
        img.add(imgTemp);
     ...
     * Constructor.
     */
    LemmingResource(final LemImage sourceImg, final int animFrames) {
        frames = animFrames;
        width = sourceImg.getWidth();
        height = sourceImg.getHeight() / animFrames;
        dirs = 1;
        animMode = Lemming.Animation.NONE;
        List<LemImage> anim = ToolBox.getAnimation(sourceImg, animFrames);
        img.add(anim);
     ...
     * Constructor.
     */
    LemmingResource(final LemImage sourceImg, final LemImage sourceImgLeft, final int animFrames) {
        frames = animFrames;
        ...
        dirs = 2;
        animMode = Lemming.Animation.NONE;
        List<LemImage> animRight = ToolBox.getAnimation(sourceImg, animFrames);
        List<LemImage> animLeft = ToolBox.getAnimation(sourceImgLeft, animFrames);
        img.add(animRight);
        img.add(animLeft);
     ...
     */
    LemImage getImage(final Lemming.Direction dir, final int frame) {
        if (dirs > 1) {
    ...
    void replaceColors(final int templateCol, final int replaceCol,
        ...
        for (ListIterator<List<LemImage>> itd = originalColorImg.listIterator();
                itd.hasNext(); ) { // go though all directions
            int di = itd.nextIndex();
            for (ListIterator<LemImage> itf = itd.next().listIterator();
...
class ExplodeFont {
    ...
    private final List<LemImage> img;
     ...
     */
    LemImage getImage(final int num) {
        return img.get(num);
    }
}